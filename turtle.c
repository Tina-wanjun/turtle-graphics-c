...
int validCmd(char cmd[10]){
  ...
  char *sample[7] = { "penup", "pendown", "colour", "forward",
  "backward", "right", "left" };
...
void printCommand(CmdNode *node, int counter){
  if(withValue(node)){
      printf("%3d: %s %d\n", counter, node->cmd, node->val);
    }
  else{
    printf("%3d: %s\n", counter, node->cmd);
  }
...
CmdNode *insertCommandBefore(CmdNode *head, CmdNode *new_CmdNode, int cmdNum) {
   ...
   * TODO: Implement this function
   */
  int len_list = countCommands(head);
  if(cmdNum < 0 || cmdNum >= len_list){
  ...
  CmdNode *forward = head;
  for(int i = 0; i<cmdNum-1; i++){
    forward = forward->next;
  }
  new_CmdNode->next = forward->next;
  forward->next = new_CmdNode;
  return head;
}
...
void run(Image *im, CmdNode *head, int *endX, int *endY) {
  ...
  *endY = 0;
  ...
  while(head != NULL){
    //keep and refresh the initial (x, y)
    int initial_x = *endX;
    int initial_y = *endY;
    //find if need to use drawline()
    ...
    //4 directions for forward/backward and get new endX and endY
